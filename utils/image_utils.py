...
def draw_rounded_rectangle(draw, box, radius, outline, width):
    ...
    # Clamp radius so that it doesn't exceed half of width/height
    r = min(radius, w // 2, h // 2)

    # Prefer to use the built-in rounded_rectangle if available
    if hasattr(draw, "rounded_rectangle"):
        draw.rounded_rectangle([x1, y1, x2, y2], radius=r, outline=outline, width=width)
        return

    # Fallback: draw arcs and lines for corners
    # Arcs
    if r > 0:
        ...
        # No radius: draw a plain rectangle
...
def draw_bounding_boxes(image_path, filtered_objects):
    ...
    # Attempt to load Arial, otherwise fall back to default font
    try:
    ...
    for num, lbl, box in filtered_objects:
        ...
        # Calculate text size robustly across Pillow versions
        def _get_text_size(draw_obj, text, font_obj):
            # Prefer ImageDraw.textbbox (available in newer Pillow versions)
            if hasattr(draw_obj, "textbbox"):
                l, t, r, b = draw_obj.textbbox((0, 0), text, font=font_obj)
                return (r - l, b - t)
            # Older Pillow versions might still have textsize
            if hasattr(draw_obj, "textsize"):
                return draw_obj.textsize(text, font=font_obj)
            # Fallback to ImageFont methods if available
            ...
            return (len(text) * (font_obj.size if hasattr(font_obj, "size") else 10), font_obj.size if hasattr(font_obj, "size") else 10)

        text_width, text_height = _get_text_size(draw, lbl, font)

        # Position for label box
        img_w, img_h = final_img.size
        label_x1 = int(x1)
        label_x2 = int(x1 + text_width + 2 * padding)
        # Adjust if label box extends beyond image width
        if label_x2 > img_w - padding:
            label_x2 = img_w - padding
            label_x1 = max(padding, label_x2 - int(text_width + 2 * padding))

        # If not enough space above the bbox, place label below the bbox
        if y1 - text_height - 2 * padding < 0:
            label_y1 = int(y1)
            label_y2 = int(y1 + text_height + 2 * padding)
            text_y = int(y1 + padding)
        else:
            label_y1 = int(y1 - text_height - 2 * padding)
            label_y2 = int(y1)
            text_y = int(y1 - text_height - padding)

        label_box = [label_x1, label_y1, label_x2, label_y2]

        # Draw label box. If we have RGBA and want transparency, we need to overlay
        try:
            # If image supports alpha or we can use an RGBA overlay for transparency
            overlay = Image.new("RGBA", final_img.size, (255, 255, 255, 0))
            overlay_draw = ImageDraw.Draw(overlay)
            overlay_draw.rectangle(label_box, fill=label_fill_color)
            final_img = Image.alpha_composite(final_img.convert("RGBA"), overlay).convert("RGB")
            draw = ImageDraw.Draw(final_img)
        except Exception:
            draw.rectangle(label_box, fill=label_fill_color[:3] if isinstance(label_fill_color, tuple) else label_fill_color)

        # Draw text
        text_x = int(label_x1 + padding)
